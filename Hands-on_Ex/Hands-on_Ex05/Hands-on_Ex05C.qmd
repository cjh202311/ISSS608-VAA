...
```{r}
pacman::p_load(seriation, dendextend, heatmaply, tidyverse,viridis,plotly)
```
...
::: {.callout-tip titile="What I did..."}
1.  **Custom Color Palette**: Created a custom color gradient using **`colorRampPalette`** and applied it to the heatmap using the **`col`** parameter. This customization provides a more personalized and visually appealing color scheme.

2.  **Font Size Adjustment**: Adjusted the font size of the main title (**`cex.main`**) and axis labels (**`cex.lab`**). This enhances readability and makes the visual elements more proportionate.

3.  **Adjustment of Graph Height**: Set the height of the entire R code block's figure using **`{r fig.height=10}`**, which might be helpful for better layout and presentation in a document.
:::
